...
module.exports = function (grunt) {
    grunt.initConfig({
        ...
        exec: {
            ...
            publish_task: {
                command: "tfx build tasks upload --token <%= settings.publish.token %> --auth-type pat --task-path ./DemoTask --service-url <%= settings.serviceUrl %>",
                stdout: true,
                stderr: true
            },
            publish_ext: {
        ...
        copy: {
            main: {
                files: [
                    { expand: true, cwd: 'node_modules/vsts-task-sdk/', src: ['VstsTaskSdk/**'], dest: 'DemoTask/ps_modules' }]
            }
        },
        jasmine: {
            src: ["scripts/**/*.js"],
